...
class Data_Handling(Data_Scraping):
    ...
    def modeling_prep_mtg_oracle(self, df):
        ...
        # Creates target vector and drops from feature matrix. Target is related cards.    
    ...
    def lemma(self, df: pd.DataFrame):
        ...
        for doc in self.df['oracle_text']:
            lemmas.append([token.lemma_.lower().strip() for token in nlp(str(doc)) if ((token.is_stop == False) 
